...
Maths               = Term ( "plus" | "minus" | "mul" | "div" ) Term .

TermOrMaths         = ( Maths | "(" Maths ")" | Term ) .

Equality            = TermOrMaths ( "eq" | "neq" ) TermOrMaths .

Comparison          = TermOrMaths ( "gt" | "gte" | "lt" | "lte" ) TermOrMaths .

Range               = TermOrMaths "between" TermOrMaths "and" TermOrMaths .

Matching            = Term ( "startswith" | "endswith" | "contains" ) Term .
