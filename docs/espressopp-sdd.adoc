...
:imagesdir: {docdir}/assets/espressopp-sdd
:espressopp: Espresso++
...
The result of this separation is the ability to add new operations to existing structures
...
A filter consists of one or more statements written in {espressopp} <<els>> that abstracts away
...
{espressopp} expressions into native expressions expected by the underlying data management system.
The next sections describe what functionality {espressopp} shall provide and how that functionality
...
image::key-elements.png[Class Diagram]
...
===== Scenario: Interpret {espressopp} Scritp

The sequence diagram depicted in figure <<interpret-esspressopp-script>> describes how an
{espressopp} script is interpreted into a native query.

[[interpret-espressopp-script]]
.Scenario Interpret {espressopp} Script
image::interpret-espressopp-script.png[Sequence Diagram]
...
`Interpreter` -- this construct allows the `CodeGenerator` to access the `Parser` and get back the
{espressopp} _grammar_.
...
{espressopp} command-line utility and the mapping of resources onto this process.

[[process-composition]]
.Process Composition
image::process-composition.png[Class Diagram]

The {espressopp} command-line utility uses the `Interpreter` and `CodeGenerator` exactly the same
way client applications do. It is meant to help developers debug filters written in {espressopp}
script.
